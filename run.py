    ...
    try:
        ...
        with Connection() as connection:
            ...
            for _, row in employees_csv_data.iterrows():
                connection.insert_data('employees', row.to_dict())

            for _, row in json_df.iterrows():
                connection.insert_data('employees', row.to_dict())

            for row in xml_data:
                connection.insert_data('employees', row)

            # Insert data into 'timerecord' table
            for _, row in timerecord_csv_data.iterrows():
                connection.insert_data('timerecord', row.to_dict())
...
def create_tables():
    try:
        with Connection() as connection:
            connection.create_tables_from_sql_file("sql\employee.sql")
            connection.create_tables_from_sql_file("sql/timesheet.sql")
